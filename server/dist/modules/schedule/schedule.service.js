...
export const getAvailableSlots = async (doctorId, date) => {
    ...
    const allSlots = generateSlots(schedule.startTime, schedule.endTime, schedule.slotDuration, schedule.bufferTime);
...
export const upsertSchedule = async (userId, data) => {
    const doctor = await db.doctorProfile.findUnique({ where: { userId } });
    if (!doctor)
        throw new AppError("Doctor profile not found. Please create your profile first.", 404);
    // Validate times
